...
@bot.event
async def on_ready():
    print(f'Giriş yapildi:  {bot.user.name}')  # Botun adını konsola çıktı olarak verir
...
async def attack(ctx):
    ...
    if target:  # Kullanıcının belirtilip belirtilmediğini kontrol ederiz
        ...
        if target.name in Pokemon.pokemons and ctx.author.name in Pokemon.pokemons:
            ...
            await ctx.send("Savaş için her iki takkimin da Pokémon sahibi olmasi lazimdir gerekir!")  # Katılımcılardan birinin Pokémon'u yoksa bilgilendiririz
    else:
       
     await ctx.send("Saldiracağiniz kullaniciyi etiketleyerek belirtin.")  # Saldırmak için kullanıcıyı etiketleyerek belirtmesini isteriz     
